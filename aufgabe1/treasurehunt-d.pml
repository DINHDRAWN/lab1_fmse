...
bool profStrategy = true; // prof Strategie wird gefolgt
bool keyTreasure = true;
proctype professor() {
	...
	:: 	printf("Neue Runde");
		...
		if
		:: treasureMap[(key) % SIZE].row[(key) / SIZE] == 0 && treasureMap[(treasureChest) % SIZE].row[(treasureChest) / SIZE] != 0 ->
			keyTreasure = false;
		:: else -> skip;
		fi
...
ltl p4 {[]!(profStrategy && 
			keyTreasure &&
    		professor@exitSignal && 
    		treasureMap[(key) % SIZE].row[(key) / SIZE] == 2 &&
